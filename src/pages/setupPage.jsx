...
export default function SetupPage() {
  ...
  const fetchLeaveTypes = async () => {
    try {
      const { data } = await ep1.get("/getleavetypes", { params: { colid } });
      setLeaveTypes(data);
    } catch (e) {
      openSnack(e?.response?.data?.error || "Error fetching leave types", "error");
    }
  };

  const fetchLeaveBalances = async () => {
    try {
      const { data } = await ep1.get("/getleavebalances", { params: { colid } });
      setLeaveBalances(data);
    } catch (e) {
      openSnack(e?.response?.data?.error || "Error fetching balances", "error");
    }
  };

  const fetchApprovers = async () => {
    try {
      const { data } = await ep1.get("/getallapprovers", { params: { colid } });
      setApprovers(data);
    } catch (e) {
      openSnack(e?.response?.data?.error || "Error fetching approvers", "error");
    }
  };
  ...
  const createLeaveType = async () => {
    ...
    try {
      await ep1.post("/createleavetype", { ...leaveTypeForm, colid });
  ...
  const createLeaveBalance = async () => {
    try {
      await ep1.post("/createleavebalance", {
        ...balanceForm,
  ...
  const searchUsers = async (_, value) => {
    ...
    try {
      const { data } = await ep1.get("/searchuserbyemailorname", {
        params: { q: value, colid },
      });
      setUsers(data);
    } catch (e) {
      openSnack(e?.response?.data?.error || "User search failed", "error");
    }
  };
