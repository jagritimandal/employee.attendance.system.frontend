...
export default function LeavesPage() {
  ...
  const fetchMyLeaves = async () => {
    try {
      const { data } = await ep1.get("/getleaves", {
  ...
  const fetchApprovals = async () => {
    try {
      const [leaves, approvers] = await Promise.all([
        ep1.get("/getleaves", { params: { colid } }),
        ep1.get("/getallapprovers", { params: { colid } }),
      ]);
  ...
  const handleApplyLeave = async () => {
    ...
    try {
      await ep1.post("/createleave", { ...form, colid });
      setForm({
        email: userEmail,
        leavetype: "",
        from: "",
  ...
  const handleAction = async (id, action) => {
    try {
      await ep1.post(`/approverejectleave?id=${id}`, {
